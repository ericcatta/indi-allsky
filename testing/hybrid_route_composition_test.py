    ...
    for name in ('views.py', 'classic_views.py'):
        for node in ast.walk(ast.parse((FLASK / name).read_text())):
            if (
                ...
                if node.args[0].value in ('/modern-admin/tools/camera-simulator', '/modern-admin/tools/generate', '/modern-admin/tools/image-circle-helper', '/modern-admin/tools/process-fits'):
