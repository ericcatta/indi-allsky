...
def test_modern_loop_uses_media_camera_filter_context():
    ...
    assert_true("v='modern-loop-preview-layout-v5'" in template, 'Loop template must cache-bust the responsive layout CSS')
...
def test_modern_loop_preview_layout_contains_media_without_forced_stretch():
    ...
    assert_true('width: 100%;\n  max-width: 100%;\n  height: auto;' in css, 'Single-camera Loop preview frame must grow with the image instead of forcing 16:9')
    assert_true('.modern-admin-loop-preview-grid-multi .modern-admin-live-frame {\n  height: clamp(260px, 24vw, 460px);' in css, 'All-camera Loop preview frame must keep a bounded dashboard height')
    assert_true('aspect-ratio: 16 / 9;' not in css, 'Loop preview frame must not derive oversized width from aspect ratio')
