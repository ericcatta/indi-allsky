    ...
    def execute_legacy(self, config, payload):
        namespace = {
            ...
            'self': SimpleNamespace(
                ...
                full_config_image_calibration_parser=(
                    lambda: ModernAdminFullConfigImageCalibrationParser()
                ),
...
def test_realtime_keogram_parser_preserves_legacy_casting_and_nested_values():
    parser = ModernAdminFullConfigRealtimeKeogramParser()
    config = {'REALTIME_KEOGRAM': {'LEGACY_VALUE': 'preserve'}}
    payload = {
    ...
    for missing_field in parser.REQUIRED_FIELDS:
        ...
        incomplete_payload.pop(missing_field)
        try:
            parser.apply({'REALTIME_KEOGRAM': {}}, incomplete_payload)
        except KeyError as error:
            assert error.args == (missing_field,)
        else:
            raise AssertionError('{0:s} should remain required'.format(missing_field))
...
def test_image_calibration_parser_preserves_legacy_casting():
    parser = ModernAdminFullConfigImageCalibrationParser()
    config = {'LEGACY_VALUE': 'preserve'}
    payload = {
        'IMAGE_CALIBRATE_DARK': 'false',
        'IMAGE_CALIBRATE_BPM': 0,
        'IMAGE_CALIBRATE_FIX_HOLES': 1,
        'IMAGE_CALIBRATE_HOLE_THOLD': '30',
        'IMAGE_CALIBRATE_MANUAL_OFFSET': '-2',
...
def test_image_output_parser_preserves_legacy_casting_and_nested_values():
    parser = ModernAdminFullConfigImageOutputParser()
    config = {
        'LEGACY_VALUE': 'preserve',
        'IMAGE_FILE_COMPRESSION': {'legacy': 1},
    }
    payload = {
        'PRIVACY_MODE': 'false',
        'IMAGE_EXIF_PRIVACY': 0,
        'IMAGE_FILE_TYPE': 42,
        'IMAGE_FILE_COMPRESSION__JPG': '90',
        'IMAGE_FILE_COMPRESSION__PNG': 3,
        'IMAGE_FOLDER': 100,
        'VARLIB_FOLDER': '/var/lib/indi-allsky',
        'IMAGE_LABEL_TEMPLATE': '{timestamp}',
        'IMAGE_EXTRA_TEXT': None,
    }
    ...
    for missing_field in parser.REQUIRED_FIELDS:
        ...
        incomplete_payload.pop(missing_field)
        try:
            parser.apply({'IMAGE_FILE_COMPRESSION': {}}, incomplete_payload)
        except KeyError as error:
            assert error.args == (missing_field,)
        else:
            raise AssertionError('{0:s} should remain required'.format(missing_field))
...
def test_ajax_config_view_delegates_realtime_keogram_parsing():
    ...
    assert 'full_config_realtime_keogram_parser().apply' in parser_source
    for field_name in ModernAdminFullConfigRealtimeKeogramParser.REQUIRED_FIELDS:
        assert field_name not in harness.direct_payload_keys
...
def test_ajax_config_view_delegates_image_calibration_parsing():
    ...
    assert 'full_config_image_calibration_parser().apply' in parser_source
    for field_name in ModernAdminFullConfigImageCalibrationParser.REQUIRED_FIELDS:
        assert field_name not in harness.direct_payload_keys
...
def test_ajax_config_view_delegates_image_output_parsing():
    ...
    assert 'full_config_image_output_parser().apply' in parser_source
    for field_name in ModernAdminFullConfigImageOutputParser.REQUIRED_FIELDS:
        assert field_name not in harness.direct_payload_keys
