...
(async()=>{
 ...
 assert(links.every(link=>new URL(link.href).searchParams.get('all')==='1'));
 assert.equal(new URL(links[1].href).searchParams.get('camera_id'),'2');
 assert.equal(new URL(links[1].href).searchParams.get('profile_id'),'p2');
 assert.equal(new URL(links[1].href).searchParams.get('timestamp'),'123');
 nodes['sensor-show-all'].checked=false;nodes['sensor-show-all'].change();
 assert(links.every(link=>!new URL(link.href).searchParams.has('all')));
