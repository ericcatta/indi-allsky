...
function fixture(){
 const form={dataset:{unit:'indi.timer'},elements:{confirmed:{checked:false},command:{value:'enable'}},fields:{disabled:false},querySelector(){return this.fields;},addEventListener(e,fn){this[e]=fn;}};
...
(async()=>{
 ...
 for(const kind of ['success','error','network','redirect','html']){
  f=fixture();f.form.elements.confirmed.checked=true;
  ...
  assert.deepEqual(JSON.parse(f.calls[0].options.body),{CAMERA_ID:2,SERVICE_HIDDEN:'indi.timer',COMMAND_HIDDEN:'enable'});
