...
function fixture(command='enable'){
 const form={dataset:{unit:command!=='enable'?'system':'indi.timer'},elements:{confirmed:{checked:false},command:{value:command}},fields:{disabled:false},querySelector(){return this.fields;},addEventListener(e,fn){this[e]=fn;}};
...
(async()=>{
 ...
 for(const command of ['enable','poweroff','validate_db']) for(const kind of ['success','error','network','redirect','html']){
  ...
  assert.deepEqual(JSON.parse(f.calls[0].options.body),{CAMERA_ID:2,SERVICE_HIDDEN:command!=='enable'?'system':'indi.timer',COMMAND_HIDDEN:command});
  if(kind==='network')f.reject();else if(kind==='redirect')f.reply({redirected:true});else if(kind==='html')f.reply({json:async()=>{throw Error('html');}});else f.reply({ok:kind==='success',json:async()=>kind==='success'?{'success-message':command==='validate_db'?'<p>Images: 2</p><p>Removed 1 missing image entries</p>':'Job submitted'}:{form_global:['Unavailable']}});
  await pending;assert(f.form.fields.disabled);await f.submit();assert.equal(f.calls.length,1);
  assert(f.result.textContent.includes(kind==='success'?(command==='validate_db'?'Removed 1 missing image entries':'accepted'):kind==='error'?'Unavailable':kind==='redirect'?'Session expired':'could not be confirmed'));
 }
