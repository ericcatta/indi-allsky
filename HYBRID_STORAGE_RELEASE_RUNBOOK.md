# Combined Hybrid worker release

Prepared on 13 September 2026; **not installed**. This replaces the standalone
Mask Base release candidate. The previous acceptance window ended at 20:00;
...
- Tested candidate: `dcaa481cb8a257b1d08f31727feb575a2e9e52fa`.
- Verified release delta: 59 paths, including Mask Base, Settings, Observatory,
  System contexts, storage policy/forecast and upload publication locking.
- Regression: 148 Python results including compilation, 34 JavaScript tests;
  all passed with the 818-file source manifest unchanged.

The Raspberry has the pinned bundle, path manifest and
`~/hybrid-storage-worker-deploy.py`. Its `--prepare-only` execution finished
...
`~/hybrid-storage-worker-release-state.json` identifies the backup and revisions.
...
Never publish the private state, database or configuration contents.
...
6. Record each missing live check explicitly and finish at the agreed deadline.
