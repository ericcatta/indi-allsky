...
- Book 2 / Safe Actions Runtime:
  ...
  - l'effetto `systemctl --user start/stop/restart indi-allsky.service` resta
    ...
    isolare.
  - il planning dei comandi generazione `generate_video` e `generate_k_st`
    e' ora Hybrid-owned tramite `ModernAdminGeneratedOutputActionPlanner`;
    l'enqueue DB/task queue resta adapter Classic/Flask esistente, e
    delete/upload/combo/panorama restano fuori scope.
